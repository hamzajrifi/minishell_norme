...
t_list  *ft_check_parser(token_t **token, lexer_t *lexer, t_list *lst)
{
    ...
    *token = lexer_get_next_token(lexer, *token);
    ...
    if ((*token) && (*token)->type == t_error)
        return (print_error("minishell: syntax error 2", lexer, *token, lst));
    return (lst);
}

t_list    *print_error(char *str, lexer_t *lexer, token_t *token, t_list *lst)
{
	int i;
	t_list *head;
...
t_list  *ft_parser(char *src, t_shell *mini)
{
	...
	if(token && token->type == t_error)
		return (print_error("minishell: syntax error 3", lexer, token, lst));
	head = lst;
	while(token && token->type != t_error)
	{
		...
		if (token && lst)
		{
			if (!(lst = add_node_in_lst(token->val, token->type, head)))
				return (print_error("minishell: syntax error 4", lexer, token, lst));
			if (token->type > t_output && token->type <= t_error )
			{
				token = lexer_get_next_token(lexer, token);
				if (!(lst = add_node_in_lst(token->val, token->type, head)) || token->type == t_error)
					return (print_error("minishell: syntax error 5", lexer, token, lst));
			}
			while (lst->next)
				lst = lst->next;
		}
